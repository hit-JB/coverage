...
import numpy as np
from PSO.sko.tools import func_transformer
...
class PSO(SkoBase):
    ...
    def __init__(self, func, n_dim=None, pop=40, max_iter=150, x_lb=-1e5, x_ub=1e5,y_lb = -1e5,y_ub = 1e5, w=0.8, c1=0.5, c2=0.5,
        ...
        self.x_lb, self.x_ub = np.array(x_lb) , np.array(x_ub)
        self.y_lb,  self.y_ub =  np.array(y_lb) , np.array(y_ub)
        #assert self.n_dim == len(self.x_lb) == len(self.x_ub), 'dim == len(lb) == len(ub) is not True'
        #assert np.all(self.x_ub > self.x_lb), 'upper-bound must be greater than lower-bound'
        ...
        self.X[:,0] = np.random.uniform(low=self.x_lb, high=self.x_ub, size =self.pop)
        self.X[:,1] = np.random.uniform(low =self.y_lb,high=self.y_ub,size=self.pop)
        vx_high = (self.x_ub - self.x_lb)/self.pop
        vy_high = (self.y_ub-self.y_lb) /self.pop
        self.V = np.zeros((self.pop, self.n_dim))
        self.V[:, 0] = np.random.uniform(low=-vx_high, high=vx_high, size=self.pop)  # speed of particles
        self.V[:, 1] = np.random.uniform(low=-vy_high, high=vy_high, size=self.pop)  # speed of particles
    ...
    def update_X(self):
        ...
        self.X[:,0] = np.clip(self.X[:,0], self.x_lb, self.x_ub)
        self.X[:,1] = np.clip(self.X[:,1],self.y_lb
                              ,self.y_ub)
