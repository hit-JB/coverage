...
class PSO_rotate(SkoBase):
    ...
    def __init__(self, func, n_dim=None, pop=40, max_iter=150, x_lb=-1e5, x_ub=1e5,y_lb = -1e5,y_ub = 1e5, w=0.8, c1=0.5, c2=0.5,
        ...
        for i in range(self.n_dim):
            if i % 3 == 0:
                self.V[:, i] = np.random.uniform(low=-vx_high, high=vx_high, size=self.pop)  # speed of particles
            elif i % 3 ==1:
                self.V[:, i] = np.random.uniform(low=-vy_high, high=vy_high, size=self.pop)  # speed of particles
            else:
                self.V[:,i] = np.random.uniform(low=0,high=360,size=self.pop)
    ...
    def update_X(self):
        ...
        for i in range(self.n_dim):
            if i % 3 == 0:
                self.X[:,i] = np.clip(self.X[:,i], self.x_lb, self.x_ub)
            elif i%3 ==1:
                self.X[:,i] = np.clip(self.X[:,i],self.y_lb
                                  ,self.y_ub)
    def cal_y(self):
